...
def make_staff_table(table_name, month, year):
    ...
    df3 = oit_stsb.load_data(table=table_name, connection_string=conn_string)
    ...
    df = df.merge(oit_stsb.load_staff(connection_string=conn_string), left_on='specialist', right_on='fio', how='left')
    df.drop(['fio', 'state'], axis=1, inplace=True)
    ...
    df = df[['specialist', 'region', 1, 'tasks_receive', 'task_number', 14, 16, 18, 9]]
...
def set_staff_columns():
    columns = [{'name': ['ФИО сотрудника', ''], 'id': 0},
               {'name': ['Регион', ''], 'id': 1},
               {'name': ['Решено', 'шт.'], 'id': 2},
               {'name': ['В работе', 'шт.'], 'id': 3},
               {'name': ['Поступило', 'шт.'], 'id': 4},
               {'name': ['Иниденты, закрытые без участия 3Л', '% (шт.)', 'Не менее 70%'], 'id': 5},
               {'name': ['Инциденты, без нарушение SLA', '% (шт.)', 'не менее 85%'], 'id': 6},
               {'name': ['Инциденты, вернувшиеся на доработку', '% (шт.)', 'Не более 10%'], 'id': 7},
